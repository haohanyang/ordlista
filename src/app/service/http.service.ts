...
import { User, Word, WordList } from "../lib/model";
...
export default class HttpService {
  ...
  getLists$(userId: string) {
    return this.http.get<{ lists: WordList[] }>(
      `${this.baseUrl}/users/${userId}/lists`,
    );
  }
  ...
  updateList$(list: WordList) {
    return this.http.patch<{ list: WordList }>(
      `${this.baseUrl}/lists/${list.id}`,
      list,
    );
  }
  ...
  getWords$(listId: string) {
    return this.http.get<{ words: Word[] }>(
      `${this.baseUrl}/lists/${listId}/words`,
    );
  }
  ...
  updateWord$(word: Word) {
    return this.http.patch<{ word: Word }>(
      `${this.baseUrl}/words/${word.id}`,
      word,
    );
  }
  ...
  getPreSignedUploadUrl$(contentType: string) {
    return this.http.post<{ url: string; fields: object }>(
